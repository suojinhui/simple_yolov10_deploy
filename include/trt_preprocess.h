...
extern  void warpaffine_init(int srcH, int srcW, int tarH, int tarW);

void preprocess_resize_cpu(cv::Mat &src, const int& tarH, const int& tarW, float* m_inputMemory);
void preprocess_resize_gpu(cv::Mat &h_src, float* d_tar, const int& tar_h, const int& tar_w);
void resize_bilinear_gpu(float* d_tar, uint8_t* d_src, int tarW, int tarH, int srcH, int srcW);
